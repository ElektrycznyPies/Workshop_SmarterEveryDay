            ...
            <section class="grid" id="tables">
                <table class="striped full-width">
                    ...
                    <c:if test="${not empty packets_grey}">
                        ...
                        <tr><td colspan="5">Packets you already have</td></tr>
...
<%--        <c:otherwise>--%>
<%--        <p>Select categories of the packets you're interested in--%>
<%--&lt;%&ndash;            <a href="#" onclick="alert('These elements will be presented to you during the study session (eg.: if you want to learn birds\' names by their photos, select Image here, but not Name)'); return false;">*</a>&ndash;%&gt;--%>
<%--        </p>--%>
<%--        <form method="get" action="/flashpack/bazaar">--%>
<%--            <c:forEach var="category" items="${categories}">--%>
<%--            <input type="checkbox" name="category" value="${category.id}">${category.name}</input>--%>
...
<%--&lt;%&ndash;        <form action="<c:url value='/flashpack/user/packets/${packetId}/update-study-settings'/>" method="post">&ndash;%&gt;--%>
<%--&lt;%&ndash;            <div class="grid container">&ndash;%&gt;--%>
<%--&lt;%&ndash;                <div><label><input type="checkbox" name="showFields" value="name" ${showFields.contains('name') ? 'checked' : ''}> Name</label></div>&ndash;%&gt;--%>
<%--&lt;%&ndash;                <div><label><input type="checkbox" name="showFields" value="word" ${showFields.contains('word') ? 'checked' : ''}> Word 1</label></div>&ndash;%&gt;--%>
<%--&lt;%&ndash;                <div><label><input type="checkbox" name="showFields" value="word2" ${showFields.contains('word2') ? 'checked' : ''}> Word 2</label></div>&ndash;%&gt;--%>
<%--&lt;%&ndash;                <div><label><input type="checkbox" name="showFields" value="additionalText" ${showFields.contains('additionalText') ? 'checked' : ''}> Additional text</label></div>&ndash;%&gt;--%>
<%--&lt;%&ndash;                <div><label><input type="checkbox" name="showFields" value="imageLink" ${showFields.contains('imageLink') ? 'checked' : ''}> Image</label></div>&ndash;%&gt;--%>
<%--&lt;%&ndash;            </div>&ndash;%&gt;--%>
<%--&lt;%&ndash;        </form>&ndash;%&gt;--%>
<%--            <section class="grid" id="tables">--%>
<%--                <table class="striped full-width">--%>
...
<%--                    <c:if test="${not empty packets_grey}">--%>
<%--                            <hr>--%>
<%--                    <tr><td colspan="5">Packets you already have</td>--%>
<%--                    </tr>--%>
<%--                    <c:forEach items="${packets_grey}" var="packet_grey" varStatus="status">--%>
<%--                        <tr>--%>
<%--                            <td><c:out value="${packet_grey.name}"/></td>--%>
